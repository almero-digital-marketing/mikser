...
module.exports = function(mikser) {
	...
	function send(message, workers) {
		...
		if (workers) {
			...
			if (workers.length === 1) {
				return sendToWorker(workers[0]);
			}
			return Promise.map(workers, sendToWorker);
		} else {
			return new Promise((resolve, reject) => {
				message._id = uuid.v1();
		...
		mikser.on('mikser.workersInitialized', () => {
			for (let worker of mikser.workers) {
				worker.on('message', (message) => {
					// console.log('W', message.call);
					return receive(message, worker)
				});	
			}
		});
	} else {
		process.on('message', (message) => {
			// console.log('M', message.call);
