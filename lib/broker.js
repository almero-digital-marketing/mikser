...
function init(mikser) {
	...
	function receive(message, worker) {
		if (message._id) {
			if (broker.pending[message._id]) {
				...
				delete broker.pendingInfo[message._id];
		...
		if (workers) {
			return Promise.map(workers, (worker) => {
				return new Promise((resolve, reject) => {
					...
					broker.pendingInfo[message._id] = message.call;
					broker.pending[message._id] = (message) => {
	...
	mikser.on('mikser.stoppingWorkers', () => {
		if (_.keys(mikser.broker.pending).length) {
			for (let pendingId in mikser.broker.pending) {
				console.log(mikser.broker.pendingInfo[pendingId].call);
			}
