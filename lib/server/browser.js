	...
	return (app) => {
		...
		if (mikser.config.browser) {
			...
			let modules = _.uniq(mikser.config.browser).map((pluginName) => {
				...
				let pluginModules = path.join(plugin, 'node_modules');
				if (fs.existsSync(pluginModules)) {
					app.use('/mikser/node_modules', express.static(pluginModules));
				}
			...
			browserifySettings('transform', [path.join(__dirname, '../../node_modules/browserify-css')], {global: true});
