	...
	return (app) => {
		...
		if (mikser.config.browser) {
			...
			let modules = _.uniq(mikser.config.browser).map((pluginName) => {
				...
				let pluginModules = plugin;
				if (pluginModules.indexOf('.js') > -1) {
					pluginModules = path.dirname(pluginModules);
				}
				if (pluginModules.indexOf(path.join(mikser.options.workingFolder, 'browser')) == -1 &&
					pluginModules.indexOf(path.join(__dirname, '..', '..', 'browser')) == -1 &&
					fs.existsSync(pluginModules)) {
					app.use('/mikser/browser/' + pluginName.replace('/browser',''), express.static(pluginModules));
				}
			...
			let jqueryModule = {};
			jqueryModule[require.resolve('jquery')] = { expose: 'jquery' };
			modules.unshift(jqueryModule);
