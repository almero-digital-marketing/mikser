...
module.exports = function(mikser) {
	...
	function getEntityInfo(collection, entityId) {
		...
		if (externalId) {
			sourceStat = fs.statSync(path.join(mikser.config[collection + 'Folder'], externalId));
		}
	...
	manager.importDocument = function (documentId, database) {
		...
		let externalId;
		if (mikser.parser.findEngine(documentId)) {
			externalId = manager.getDocumentId(documentId);
			if (externalId) {
				let tempId = documentId;
		...
		if (externalId) {
			externalId = S(externalId).replaceAll('\\','/').ensureLeft('/').s;
			sourceStat = fs.statSync(path.join(mikser.config.documentsFolder, externalId));
		}
		...
		let validateDestination = (database) => {
			if (!document.destination) return Promise.resolve(true);
			return database.documents.findOne({ 
				$and: [
		...
		if (!database) {
			return using(mikser.database.connect(), (database) => {
				return validateDestination(database).then((valid) => {
					if (valid) return inspect(database);
					return Promise.resolve();
				});
			});
		}
	...
	manager.importLayout = function (layoutId, database) {
		...
		if (!mikser.generator.findEngine(layoutId)) {
			externalId = manager.getLayoutId(layoutId);
			if (externalId) {
				let tempId = layoutId;
				layoutId = externalId;
				externalId = tempId;
		...
		if (externalId) {
			externalId = S(externalId).replaceAll('\\','/').ensureLeft('/').s;
			sourceStat = fs.statSync(path.join(mikser.config.layoutsFolder, externalId));
		}
	...
	manager.importView = function (viewId) {
		...
		return mikser.emit('mikser.manager.importView', view).then(() => {
			return database.views.save(view).then(() => {
	...
	manager.deleteLayout = function (layoutId) {
		...
		if (externalId) {
			...
			return using(mikser.database.connect(), (database) => {
				return database.findLayout({_id: layoutId}).then((layout) => {
	...
	manager.deleteView = function (viewId) {
		...
		if (!mikser.generator.findEngine(viewId)) {
			externalId = manager.getViewId(viewId);
		...
		if (externalId) {
			return using(mikser.database.connect(), (database) => {
				return database.views.findOne({
					_id: viewId
				}).then((view) => {
					if (view && view.destination) {
						return fs.removeAsync(view.destination);
					}
				}).then(() => {
					return database.views.remove({_id: viewId});
				}).then(() => {
			...
			return using(mikser.database.connect(), (database) => {
				return database.views.findOne({_id: viewId}).then((view) => {
					if (!view) Promise.resolve();
					return mikser.emit('mikser.manager.deleteView', view).then(() => {
						return mikser.runtime.cleanLinks({_id: viewId}, database).then(() => {
							return database.views.remove({_id: viewId});
						}).then(() => {
							return mikser.runtime.followLinks(view, database, true);
						}).then(() => {
