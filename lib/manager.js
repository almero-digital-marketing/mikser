...
module.exports = function(mikser) {
	...
	manager.importLayout = function (layoutId, database) {
		...
		let externalId = layoutId;
		if (!mikser.generator.findEngine(layoutId)) {
			let externalSource = path.join(mikser.config.layoutsFolder, externalId);
			let externalBasename = path.basename(externalSource);
			let externalExt = path.extname(externalSource);
			let externalName = externalBasename.replace(externalExt, '');

			if (fs.statSync(externalSource).isDirectory()) return Promise.resolve();

			let dirContent = fs.readdirSync(path.dirname(externalSource));
			dirContent = dirContent.filter((fileName) => {
				if (fileName === externalBasename) return false;
				return fileName.replace(path.extname(fileName), '') === externalName;
			});
			if (!dirContent.length) return Promise.resolve();

			let engine;
			for (let file of dirContent) {
				if (engine) break;
				engine = mikser.generator.findEngine(file);
				if (engine) {
					layoutId = externalId.replace(externalExt, path.extname(file));
				}
			}
		}
