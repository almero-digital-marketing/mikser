...
module.exports = function(mikser) {
	...
	if (mikser.config.extensions) filemanager.extensions = _.defaultsDeep(mikser.config.extensions, filemanager.extensions);
	...
	filemanager.predictDestination = function (file, info) {
		...
		if (file.indexOf(mikser.config.documentsFolder) === 0 && 
			...
			if (info.markup) {
				...
				destinationExt = mikser.filemanager.extensions[sourceExt] || destinationExt;
			}
	...
	filemanager.importDocument = function (documentId, database) {
		...
		let destination = filemanager.predictDestination(source, info);
		let document = {
			_id: documentId, 
			stamp: mikser.stamp,
			importDate: new Date(),
		...
		mikser.emit('mikser.filemanager.importDocument', document);
	...
	filemanager.importLayout = function (layoutId, database) {
		...
		let inspect = (database) => {
			return database.layouts.findOne({_id: layoutId}).then((layout) => {
				...
				mikser.emit('mikser.filemanager.importLayout', layout);
				if (addToscheduler) {
	...
	filemanager.deleteDocument = function (documentId) {
		...
		return using(mikser.database.connect(), (database) => {
			return database.documents.findOne({_id: documentId}).then((document) => {
				...
				mikser.emit('mikser.filemanager.deleteDocument', document);
	...
	filemanager.deleteLayout = function (layoutId) {
		...
		return using(mikser.database.connect(), (database) => {
			return mikser.database.findLayout({_id: layoutId}).then((layout) => {
				...
				mikser.emit('mikser.filemanager.deleteLayout', layout);
