...
module.exports = function(mikser) {
	...
	diagnostics.log = function() {
		...
		if (args[0].document && args.length >= 3) {
			...
			console.log(
				S(message).padRight(Math.max(bufferSize, message.length)).s);
	...
	if (cluster.isMaster) {
		...
		diagnostics.inspect = function(documentId) {			
			...
			for (let contextId in diagnostics.renderPipeline[documentId]) {
				...
				if (pipe.context.layout) {
					...
					if (pipe.context.document) {
						diagnose = prefix + ' ' + pipe.context.document._id;					
					} else {
						diagnose = prefix + ' ' + documentId;						
					}
				}
		...
		diagnostics.flush = function() {			
			...
			for (let documentId in diagnostics.renderPipeline) {
				let potentialError = diagnostics.inspect(documentId);
