...
function init(mikser) {
	...
	return new Promise((resolve, reject) => {
		...
		if (cluster.isMaster) {
			...
			mikser.receive({
				'diagnostics.splice': (message) => diagnostics.splice(message.context),
				'diagnostics.break': (message) => diagnostics.break(message.context),
				'diagnostics.leave': (message) => diagnostics.leave(message.context),
				'diagnostics.log': (message) => diagnostics.log(message.context, message.level, message.message),
				'diagnostics.start': (message) => diagnostics.start(message.documentId),
				'diagnostics.end': (message) => diagnostics.end(message.documentId),
			});
		}
		resolve(mikser);
	});
