...
module.exports = function(mikser) {
	...
	var diagnosticsQueue = new Queue(1, Infinity);

	scheduler.scheduleAllDocuments = function() {
		console.log('Enqueue all documents');
		return mikser.database.documents.find({destination: { $ne: false }},{ _id:1 }).toArray().then((documents) => {
			...
			if (cluster.isMaster) {
				...
				let renderDocument = function(documentId, cursor) {
					...
					return renderDocument.then(() => {
						...
						return diagnosticsQueue.add(() => {
							let inspect = Promise.resolve(false);
							if (useWorker) {
								inspect = mikser.broker.call('mikser.diagnostics.inspect', mikser.workers[cursor], documentId)
									.then((flushed) => error.flushed = flushed);
							} else {
								inspect = mikser.diagnostics.inspect(documentId);
							}
							return inspect.then((potentialError) => {
								if (potentialError) {
									error.flushed = true;
								}
				...
				if (mikser.plugins.livereload && mikser.plugins.livereload.isLive('documents', documentId)) {
				...
				if(state.depth == 1) {
					...
					return Promise.map(state.documents, (document) => {
						...
						return Promise.map(state.views, (view) => {
							if (mikser.plugins.livereload) {
								return mikser.plugins.livereload.refresh('views', view._id);
							}
						});
					}).then(() => {
						return Promise.resolve(true);						
		...
		scheduler.process = function () {
			...
			return mikser.startWorkers().then(() => {
				...
				if (!scheduler.pending) {
					...
					return mikser.stopWorkers().then(mikser.tools.build);
				}
				...
				scheduler.processing = mikser.debug.resetWatch().then(() => {
					...
					return mikser.observer.inspectData();
				}).then(() => {
					return mikser.queue.start().finally(() => {
						scheduler.pending = false;
						return mikser.diagnostics.flush().then((flushed) => {
							return mikser.broker.broadcast('mikser.diagnostics.flush').then((result) => {
								for(let workerResult of result){
									flushed = _.union(flushed, workerResult);
								}
