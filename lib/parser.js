...
module.exports = function(mikser) {
	...
	parser.findEngine = function(file) {
		for (let engine of mikser.parser.engines) {
	...
	parser.parse = function(source) {
		...
		if (engine) {
			...
			if (delimiter) {
				...
				let engine = parser.findEngine('lang.' + (lang.trim()) || parser.defaultEngine;
				if (engine) {
	...
	parser.defaultEngine = parser.findEngine(mikser.options.config);
	if (mikser.config.frontmatter) {
		parser.defaultEngine = parser.findEngine('lang.' + mikser.config.frontmatter);
	}
