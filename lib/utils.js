...
module.exports = function(mikser) {
	...
	utils.findSource = function (source) {
		...
		if (fs.existsSync(path.join(mikser.config.runtimeFolder, 'shared', source))) {
	...
	utils.predictDestination = function (file, info) {
		...
		if (file.indexOf(mikser.config.documentsFolder) === 0 && 
			...
			if (mikser.config.cleanUrls && !S(destination).endsWith('index.html') && S(destination).endsWith('.html')) {
	...
	utils.getUrl = function (destination) {
		...
		if (destination) {
			if (destination.indexOf(mikser.config.outputFolder) === 0) {
				...
				if (mikser.config.cleanUrls && !S(url).endsWith('index.html') && S(url).endsWith('.html')) {
