...
module.exports = function(mikser) {
	...
	runtime._importDocument = function(document, database) {
		return mikser.runtime.save(document).then(() => {
			return database.documents.save(document).then(() => {
				return mikser.scheduler.enqueueDocument(document._id, constants.RENDER_STRATEGY_FULL);
			});
		})
	};
	...
	runtime.importDocument = function(document, database) {
		if (!database) {
			return using(mikser.database.connect(), (database) => {
				return runtime._importDocument(document, database);
			});
		}
		return runtime._importDocument(document, database);
	};
					...
					return database.documents.find({
						...
						return Promise.map(otherPages, (otherPage) => {
							if (otherPage.pageNumber == 0) {
								return mikser.scheduler.enqueueDocument(otherPage._id, constants.RENDER_STRATEGY_STANDALONE);
							}
						});
					});
				});
