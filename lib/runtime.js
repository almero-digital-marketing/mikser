...
module.exports = function(mikser) {
	...
	runtime._link = function(document) {
		...
		return runtime._unlink(document).then(() => {
			...
			return mikser.emit('mikser.runtime.link', document);
		});

	};

	runtime._unlink = function(document) {
		...
		return mikser.emit('mikser.runtime.unlink', document);
	};
	...
	runtime.link = function(document) {
		return mikser.broker.broadcast('mikser.runtime._link', document);
	};

	runtime.unlink = function(document) {
		return mikser.broker.broadcast('mikser.runtime._unlink', document);
	};
	...
	runtime.addLinks = function(document, documentLinks, layoutLinks, database) {
		...
		if (!fs.existsSync(privatePlugin)) {
			...
			if (fs.existsSync(buildinPlugin)) {
				...
				if (fs.existsSync(publicPlugin)) {
					...
					if (S(plugin).endsWith('/index.js')) throw 'Plugin not found.';
	...
	runtime.findBrowserPlugin = function(plugin) {
		...
		if (mikser.config) {
			plugin = plugin || mikser.config[plugin];
			privatePlugin = path.join(mikser.config.browserFolder, S(plugin).ensureRight('.js').s);
		}
		if (!fs.existsSync(privatePlugin)) {
			let buildinPlugin = path.join(__dirname, '../browser', S(plugin).ensureRight('.js').s);
			if (fs.existsSync(buildinPlugin)) {
				plugin = buildinPlugin;
			}
			else {
				...
				if (fs.existsSync(publicPlugin)) {
					...
					if (S(plugin).endsWith('/index.js')) throw 'Plugin not found.';
					return runtime.findBrowserPlugin(S(plugin).ensureRight('/index.js').s);
				}
			}
		}
		else {
