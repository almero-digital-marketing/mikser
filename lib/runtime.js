...
module.exports = function(mikser) {
	...
	runtime.findHref = function(entity, href, lang) {
		...
		if (mikser.config.cleanUrls) {
			let clean = new RegExp('index.html',"gi");
	...
	runtime.findUrl = function (url) {
		// console.log(mikser.state.urlmap)
	...
	runtime.fromCache = function(name, loadData) {
		let cached = runtime.cache.get(name);
		if (cached) return Promise.resolve(cached);
		return loadData().then((data) => {
			runtime.cache.set(name, data);
	...
	if (cluster.isMaster) {
		mikser.cli
			...
			.init();
		mikser.options.keepRuntime = mikser.cli.keep;
		...
		if (fs.existsSync(recentPackage)) {
			...
			runtime.versionChange = {
				major: recentVersion[0] != currentVersion[0],
				minor: recentVersion[1] != currentVersion[1] || recentVersion[0] != currentVersion[0],
				patch: recentVersion[2] != currentVersion[2] || recentVersion[1] != currentVersion[1] || recentVersion[0] != currentVersion[0]
			}
			if (runtime.versionChange.minor) {
				fs.emptyDirSync(mikser.config.outputFolder);
			}
