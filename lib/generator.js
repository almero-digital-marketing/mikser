...
module.exports = function(mikser) {
	...
	generator.renderView = function(viewId) {
	...
	generator.renderDocument = function(documentId, strategy) {
		...
		context.href = function (href, lang, link) {
			...
			if (found && 
				...
				if (link) context.documentLink(found);
				else context.documentUnlink(found);
			}
			return found;
		}
		context.hrefPage = function (href, page, lang, link) {
		...
		let originalLinks;
		return mikser.runtime.cleanLinks(document).then((links) => {
			originalLinks = links;
