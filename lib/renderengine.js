...
function init(mikser) {
				...
				return database.findDocument({_id: documentId}).then((document) => {
					...
					return mikser.runtime.cleanLinks(document, database).then((links) => {
						...
						return mikser.loader.loadSelf(context).then((context) => {
							...
							fs.createFileSync(context.document.destination);
					...
					mikser.send({
						call: 'renderengine.documentRendered',
						documentId: documentId
					});
				}).catch((err) => {
					mikser.diagnostics.end(documentId);
					// console.log('Render['+ mikser.workerId + ']:', documentId, 'Error', err);
					let message = {
						call: 'renderengine.documentRendered',
						documentId: documentId,
						error: err.message
					};
					if (err.stack) message.error = err.stack.toString();
					mikser.send(message);
				});
			});
		} else {
			return mikser.renderqueue.enqueueDocument(documentId, strategy);
		}
	};
	...
	return new Promise((resolve, reject) => {
		...
		if (cluster.isWorker) {
			mikser.receive({
				'renderengine.renderDocument': (message) => renderengine.renderDocument(message.documentId, message.strategy)
			});
		}
