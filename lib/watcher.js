...
module.exports = function(mikser) {
	...
	if (mikser.config.watcher.output) mikser.config.watcher.output = mikser.config.watcher.output.concat(mikser.config.watcher.reload.map((glob) => '!' + glob));
			...
			if (mikser.options.watch) {
				...
				if (mikser.config.pluginsPattern && fs.existsSync(mikser.config.pluginsFolder)) {
					...
					if (mikser.config.watcher.build && !watcher.watchers.buildViews) {
						...
						watcherBuildViews.on('all', fileAction);
