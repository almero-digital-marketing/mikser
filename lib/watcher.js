...
module.exports = function(mikser) {
	...
	_.defaultsDeep(mikser.config, {
		...
		copyDelay: 3000,
		watcher: {
			output: ['**/*.json', '**/*.jpeg', '**/*.jpg', '**/*.gif', '**/*.png', '**/*.svg'],
			reload: ['**/*.css', '**/*.js'],
			files: ['**/*'],
	...
	if(cluster.isMaster) {
		...
		watcher.plug = function() {
			if (watcher.outputStack) {
				return Promise.map(_.keys(watcher.outputStack), (file) => {
					return watcher.outputStack[file]();
				}).then(() => {
					delete watcher.outputStack;
					debug('Pluged')
				});
			}
		}
			...
			if (!watching && mikser.options.watch) {
				if (mikser.config.livereload) {
					let outputAction = function(event, file) {
						let action = () => {
							return mikser.emit('mikser.watcher.outputAction', event, file).then(() => {
								debug('Reload', file);
								return mikser.server.reload(file);
							});
						};
						if (!watcher.outputStack) {
							return action();
