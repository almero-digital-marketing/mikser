	...
	function run(command, internal) {
		...
		return mikser.emit('mikser.tools.run.start', {
			...
			try {
				...
				let errorMessage = ex.error.toString();
				if (ex.stderr) {
					errorMessage = ex.stderr.toString();
				}
				mikser.diagnostics.log('error', 'Failed to start:', command, errorMessage);
				return mikser.emit('mikser.tools.run.finish', {
					command: command,
					code: ex.status
				});
		...
		if (fs.existsSync(sourceFolder)) {
			...
			if (XCOPY) {
				if (mikser.options.debug) {
					...
					sync = run('xcopy /E /D /Y /Q "' + sourceFolder +'\\*" "' + destinationFolder + '\\"', true);					
				}
			}
			else {
				if (OSX) {
