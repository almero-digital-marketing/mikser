	...
	function run(command, internal) {
		...
		return mikser.emit('mikser.tools.run.start', {
			...
			return spawn(command, args, {
				...
				mikser.diagnostics.log('error', 'Failed to start:', command, error.stderr.toString());
				return mikser.emit('mikser.tools.run.finish', {
					command, command,
					code: error.exitStatus
				});
			});
		});
	}
		...
		if (fs.existsSync(sourceFolder)) {
			...
			if (XCOPY) {
				if (mikser.options.debug) {
					sync = run('xcopy /eDy "' + sourceFolder +'\\*" "' + destinationFolder + '\\"', true);					
				} else {
					sync = run('xcopy /eDyq "' + sourceFolder +'\\*" "' + destinationFolder + '\\"', true);					
				}
			}
			else {
				if (OSX) {
