...
module.exports = function(mikser) {
	...
	tools.compile = function(file){
		...
		if (mikser.config.compile) {
			for (let compile of mikser.config.compile) {
				...
				compilation = compilation.then(() => {
					if (sync && mikser.config.shared.length) {
	...
	tools.syncFolders = function(sourceFolder, destinationFolder) {
		...
		if (fs.existsSync(sourceFolder)) {
			if (XCOPY) {
				return exec('xcopy /eDyQ "' + sourceFolder +'\\*" "' + destinationFolder + '\\"');
			}
			else {
				if (OSX) {
					return exec('rsync -a "' + sourceFolder + '/" "' + destinationFolder + '/"');
				}
				else {
					return exec('cp -Ruf "' + sourceFolder + '/." "' + destinationFolder + '"');
				}
			}
		}
