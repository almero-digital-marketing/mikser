...
module.exports = function(mikser) {
	...
	function findConfig(name) {
		...
		let configFile = path.join(mikser.options.workingFolder, name + '.json5');
		if (fs.existsSync(configFile)) return configFile;
		let configFile = path.join(mikser.options.workingFolder, name + '.toml');
		if (fs.existsSync(configFile)) return configFile;
		let configFile = path.join(mikser.options.workingFolder, name + '.cson');
		if (fs.existsSync(configFile)) return configFile;
		let configFile = path.join(mikser.options.workingFolder, name + '.yaml');
		if (fs.existsSync(configFile)) return configFile;
