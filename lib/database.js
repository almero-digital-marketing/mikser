...
module.exports = function(mikser) {
	...
	var database = {
		connect: function() {
			...
			return connect(mikser.config.databaseUrl).then((db) => {
				...
				db.views = db.collection('views');
				db.plugins = db.collection('plugins');
				db.documentLinks = db.collection('documentLinks');
				db.layoutLinks = db.collection('layoutLinks');
				
				db.findSome = function (collection, selector, orderBy) {
					if (selector.query && selector.projection) {
						if (orderBy) {
							return db.collection(collection).find(selector.query, selector.projection).sort(orderBy).toArray();
						} else {
							return db.collection(collection).find(selector.query, selector.projection).toArray();
						}
					}
					if (orderBy) {
						return db.collection(collection).find(selector).sort(orderBy).toArray();
					} else {
						return db.collection(collection).find(selector).toArray();
					}
				}
				db.findOne = function (collection, selector) {
					if (selector.query && selector.projection) {
						return db.collection(collection).findOne(selector.query, selector.projection);
					}
					return db.collection(collection).findOne(selector);
				}

				db.findDocuments = (selector, orderBy) => db.findSome('documents', selector, orderBy); 
				db.findDocument = (selector) => db.findOne('documents', selector);
				db.findLayouts = (selector, orderBy) => db.findSome('layouts', selector, orderBy); 
				db.findLayout = (selector) => db.findOne('layouts', selector);
				db.findViews = (selector, orderBy) => db.findSome('views', selector, orderBy); 
				db.findView = (selector) => db.findOne('views', selector);
				db.findPlugins = (selector, orderBy) => db.findSome('plugins', selector, orderBy); 
				db.findPlugin = (selector) => db.findOne('plugins', selector);
	...
	database.findLayouts = function(selector, orderBy) {
		return using(mikser.database.connect(), (db) => {
			return db.findLayouts(selector, orderBy);
		});
	}
	...
	database.findPlugins = function(selector, orderBy) {
		return using(mikser.database.connect(), (db) => {
			return db.findPlugins(selector, orderBy);
		});
	}

	database.findPlugin = function(selector) {
		return using(mikser.database.connect(), (db) => {
			return db.findPlugin(selector);
		});
	}

	database.findViews = function(selector, orderBy) {
		return using(mikser.database.connect(), (db) => {
			return db.findViews(selector, orderBy);
		});
	}

	database.findView = function(selector) {
		return using(mikser.database.connect(), (db) => {
			return db.findView(selector);
		});
	}
