	...
	if (cluster.isMaster) {
		function observe(entity) {
			return mikser.database.collection('entitiesObserver').find({
				...
				return mikser.database.collection('dataObserver').find().toArray().then((observations) => {
					return Promise.map(observations, (observation) => {
						if (mikser.scheduler.documentsHistory[observation.entity._id]) return;
						let subject = JSON5.parse(observation.subject);
						console.log(observation.subject);
						return mikser.database.findDocuments(subject.query, subject.orderBy).then((data) => {
							if (observation.checksum != hasha(JSON5.stringify(data), {algorithm: 'md5'})) {
								debug('Data observed:', observation.entity._id);
								return mikser.emit('mikser.observer.observe', observation.entity).then(() => {
									if (observation.entity.collection == 'documents') {
										return mikser.scheduler.scheduleDocument(observation.entity._id, constants.RENDER_STRATEGY_STANDALONE);
									}							
