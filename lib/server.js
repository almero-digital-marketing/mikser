...
module.exports = function(mikser) {
	...
	if(cluster.isMaster) {
		...
		server.isLive = function(documentId) {
			if (listening) {
				let documentUrl = mikser.state.urlmap[documentId];
				for(let clientId in mikser.server.clients) {
					if (documentUrl == mikser.server.clients[clientId].url) return true;
				}
			}
			return false;
		}
		...
		server.listen = function() {
			...
			let livereload = function (req, res, next) {
				...
				return renderDocument.then((content) => {
					let injectScript = function(content, script) {
						var snippet = '<script src="' + script + '" async></script>';
						return content.replace('</body>', snippet + '</body>');
					}
					content = injectScript(content, '/mikser.js');
					res.send(content);							
				}).catch((err) => {
			...
			app.use('/mikser.js', browserify(path.join(__dirname, '../public/index.js')));
			app.use('/mikser', lessCompiler(path.relative(process.cwd(), path.join(__dirname, '../public/assets'))));
			app.use('/mikser', express.static(path.join(__dirname, '../public/assets')));
