			...
			server.listen = function() {
				...
				if (mikser.config.livereload) {
					let injectScripts = function (req, res, next) {
						...
						if (mikser.renderqueue.processing) {
							renderDocument = mikser.database.findDocument({url: url}).then((document) => {
								return mikser.renderengine.renderDocument(document._id, constants.RENDER_STRATEGY_STANDALONE);
							});
						}
						return renderDocument.then(() => {
							return fs.readFileAsync(path.join(mikser.config.outputFolder, url), 'utf-8').then((data) => {
