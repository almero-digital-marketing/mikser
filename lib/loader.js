...
function init(mikser) {
	...
	mikser.loader.dispose = function(context) {
		if (context.layout) {
			for(let name in context.layout.meta.data) {
				if (!context.layout.meta.data[name].global) {
					debug('Offload data:', name, context.layout._id);
					delete context.data[name];
				}
			}			
			for(let name in context.layout.meta.blocks) {
				debug('Offload block:', name, context.layout._id);
						...
						return preparations.then(() => {
							context.blocks[name] = function (options) {
								...
								for (let layout of blockContext.layouts) {
									...
									mikser.renderengine.render(blockContext);
								}				
	...
	mikser.loader.loadData = function (context) {
		return Promise.map(_.keys(context.layout.meta.data), (name) => {
			...
			if (context.data[name]) return Promise.resolve(context);
				...
				if (collectionDetails.query) {
					if (typeof collectionDetails.query == 'string') {
						if (minimatch(collectionDetails.query, mikser.config.queryPattern)) {
							let source = mikser.filemanager.findSource(collectionDetails.query);
							if (source){
								collectionDetails.query = fs.readFileSync(source, { encoding: 'utf8' });
							}
						}
	...
	mikser.loader.loadPages = function (context) {
		...
		if (!context.paging && context.layout.meta.pageBy && context.layout.meta.pageSize) {
			...
			Object.defineProperty(context.paging, 'next', {
				get: function() {
					if (context.document.pageNumber < context.paging.numberOfPages - 1) {
						return context.document.meta.href.replace('/' + context.document.pageNumber,'/' + context.document.pageNumber + 1);
					} 
				}
			});
			context.paging.numberOfPages = Math.ceil(context.data[context.layout.meta.pageBy].length / context.layout.meta.pageSize);
			if (context.document.pageNumber == 0) {
				loadPages = context.database.findDocuments({
					source: context.document.source,
					pageNumber: { $gte: context.paging.numberOfPages }
				}).then((pages) => {
					return Promise.map(pages, (page) => {
						if (page.pageNumber > 0) {
							mikser.runtime.removeFromSitemap(page);
							return mikser.filemanager.deleteDocument(page._id);
						}
						return Promise.resolve();
					}, {concurrency: 1});
				}).then(() => {
					let imports = [];
					for (let pageNumber = 1; pageNumber < context.paging.numberOfPages; pageNumber++) {
						let page = extend({}, context.document);
						page.pageNumber = pageNumber;
						page._id += "." + pageNumber;
						page.meta.href = page.meta.href + '/' + pageNumber;
						...
						mikser.runtime.importInSitemap(page).then(() => {
							imports.push(context.database.documents.save(page).then(() => {
								return mikser.renderqueue.enqueueDocument(page._id, constants.RENDER_STRATEGY_FULL);
							}));							
						});
					}
